...
def _session_key() -> str:
    """Generate a meaningful unique key: YYYYMMDD_HHMMSS."""
...
def load_session(key: str) -> list:
    path = SESSIONS_DIR / f"{key}.json"
    return json.loads(path.read_text()) if path.exists() else []


def save_session(key: str, history: list) -> None:
    SESSIONS_DIR.mkdir(exist_ok=True)
    (SESSIONS_DIR / f"{key}.json").write_text(json.dumps(_serialize(history), ensure_ascii=False, indent=2))
...
if __name__ == "__main__":
    ...
    print(f"\033[90mSession: {session_key}\033[0m")
    while True:
        try:
