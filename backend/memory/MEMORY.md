...
### 内容生成任务（不是软件开发）
- 特征：做计划、写报告、生成方案 + 用网页/文档展示
- 工具优先级：web_search/cdp_browser（收集信息）→ workspace_write（生成内容）
- 示例：
  - "做一个旅游计划并用网页展示" → 生成 HTML 文件，不是开发系统
  - "写一份研究报告" → 生成 Markdown/PDF，不是开发 CMS
  - "制作产品介绍页面" → 生成静态 HTML，不是开发网站
- 关键判断：用户要的是**内容**还是**系统**？

### 系统开发任务
- 特征：开发、实现、构建 + 系统、平台、应用、数据库、API
- 工具优先级：Task(Plan) → 拆分任务 → 逐步实现
- 示例：
  - "开发一个旅游计划系统" → 需要数据库、API、前端框架
  - "构建内容管理平台" → 需要完整的软件架构
- 关键判断：需要持久化存储、用户管理、复杂交互
