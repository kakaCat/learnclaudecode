...
async def run_repl(agent, history: list):
    ...
    command_handler = CommandHandler(agent, history)
    ...
    while True:
        ...
        await _handle_agent_query(query, agent, history, task_queue)
...
async def _handle_agent_query(query: str, agent, history: list, task_queue: TaskQueue):
    """
