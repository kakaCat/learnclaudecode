    ...
    def __init__(self):
        super().__init__(
            ...
            prompt=(
                ...
                "  * memory_search - 召回之前的浏览经验和反爬策略\n\n"
                "- **Orient（定向）**: 分析页面内容，识别关键信息和缺失部分\n"
                "  * 页面是否加载完整？\n"
                "  * 是否遇到反爬虫机制（验证码、滑块、403/429错误）？\n"
                "  * 是否需要交互（点击、滚动）？\n"
                "  * 信息是否足够？\n\n"
                "- **Decide（决策）**: 选择下一步行动\n"
                "  * 继续观察（访问更多页面）\n"
                ...
                "反爬虫处理策略：\n"
                "1. **验证码/滑块**: 无法自动处理，应切换到其他网站或API接口\n"
                "2. **403/429错误**: 等待后重试，或更换User-Agent\n"
                "3. **页面加载慢**: 增加等待时间（wait参数）\n"
                "4. **动态内容**: 使用scroll或wait_for等待内容加载\n"
                "5. **备选方案**: 遇到反爬时，优先考虑：\n"
                "   - 切换到其他同类网站\n"
                "   - 使用公开API接口\n"
                "   - 调整查询参数（如日期范围）\n"
                ...
                "- CDP不可用时明确说明原因\n"
                "- 遇到验证码时不要无限重试，应切换策略\n"
                "- 保持循环直到任务完成或遇到无法绕过的限制\n\n"
                "**关键：任务完成时必须返回结构化结果**\n"
                "任务结束时，你的最终输出必须包含以下JSON格式的结果摘要：\n\n"
                "```json\n"
                "{\n"
                "  \"status\": \"success\" | \"partial\" | \"failed\",\n"
                "  \"data_count\": <提取的数据条数>,\n"
                "  \"workspace_file\": \"<保存的文件路径>\",\n"
                "  \"summary\": \"<简短总结：成功提取了什么数据，或失败原因>\",\n"
                "  \"limitations\": [\"<遇到的限制列表>\"]\n"
                "}\n"
                "```\n\n"
                "状态定义：\n"
                "- **success**: 完整提取了所有要求的数据（如：10条航班信息，包含航班号、价格等）\n"
                "- **partial**: 提取了部分数据，但不完整（如：只有价格范围，没有具体航班）\n"
                "- **failed**: 完全失败，没有提取到任何有用数据\n\n"
                "示例1（成功）：\n"
                "```json\n"
                "{\n"
                "  \"status\": \"success\",\n"
                "  \"data_count\": 12,\n"
                "  \"workspace_file\": \"北京到厦门航班信息_2026-03-15.md\",\n"
                "  \"summary\": \"成功提取12条航班信息，包含航班号、时间、价格\",\n"
                "  \"limitations\": []\n"
                "}\n"
                "```\n\n"
                "示例2（部分成功）：\n"
                ...
                "  \"data_count\": 2,\n"
                "  \"workspace_file\": \"北京到厦门航班信息_2026-03-15.md\",\n"
                "  \"summary\": \"仅从百度搜索提取到2条往返套票价格，缺少具体航班号和时间\",\n"
                "  \"limitations\": [\"飞猪/携程/去哪儿均有反爬虫机制\", \"查询日期太远，航班未开放预订\"]\n"
                "}\n"
                "```\n\n"
                "示例3（失败）：\n"
                "```json\n"
                ...
                "  \"data_count\": 0,\n"
                "  \"workspace_file\": null,\n"
                "  \"summary\": \"所有机票网站均无法访问，遇到验证码或403错误\",\n"
                "  \"limitations\": [\"反爬虫机制\", \"需要登录\", \"验证码\"]\n"
                "}\n"
                "```\n\n"
                "**重要**：即使任务失败，也必须返回这个JSON结构，让mainagent知道具体失败原因。"
            ),
            loop_type="ooda",
            max_cycles=200,
            enable_memory=True,
