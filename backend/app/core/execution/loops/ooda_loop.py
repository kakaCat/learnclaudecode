...
class OODALoop(BaseLoop):
    ...
    def _execute_loop(
        ...
        # 为 LLM 调用配置 callback
        llm_config = {"callbacks": [langchain_callback]}
        ...
        for cycle in range(1, max_cycles + 1):
            ...
            obs_tools = self._observe_phase(
                llm, system_prompt, user_prompt, observations, tool_map, llm_config
            )
            if obs_tools:
            ...
            situation = self._orient_phase(
                llm, system_prompt, user_prompt, observations, llm_config
            )
            ...
            decision = self._decide_phase(
                llm, system_prompt, user_prompt, situation, llm_config
            )
            ...
            if choice == "ACT":
                act_tools = self._act_phase(
                    llm, system_prompt, user_prompt, situation, tool_map, llm_config
                )
                if act_tools:
        ...
        output = self._generate_summary(
            llm, system_prompt, user_prompt, observations, history, llm_config
        )
